...
typedef struct proof_t {
    uint8_t seed1[32];
    uint8_t seed2[32];
    uint32_t inputShare[8];     // Input share of the party which does not derive it from the seed (not included if challenge is 0)
    uint8_t communicatedBits[143];
    uint8_t view3Commitment[64];
    uint8_t view3UnruhG;     // we include the max length, but we will only serialize the bytes we use
} proof_t;
...
typedef struct signature_t {
    proof_t proofs[438];
    uint8_t challengeBits[110];     // has length numBytes(numMPCRounds*2)
